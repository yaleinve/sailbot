...
class Captain():
    ...
    def publish_captain(self):
        ...
        leg_info.leg_course = gpsBearing(self.beginLat, self.beginLong, self.current_target_waypoint.wlat,
                                         self.current_target_waypoint.wlong)
    ...
    def getNextWaypoint(self):
        rospy.loginfo("[captain] getNextWaypoint()")
    ...
    def checkLeg(self):
        ...
        # If we have a waypoint, how far away are we from it?
            ...
            d = gpsDistance(self.currentLat, self.currentLong, self.current_target_waypoint.wlat,
                            self.current_target_waypoint.wlong)  # Distance to waypoint
    ...
    def loadLegQueue(self):
        ...
        # Alg sketch: start, sail diamond around mark, sail to x meters above line, sail straight down through
            ...
            w4.logWaypoint()
            w5 = Waypoint(loc5[0], loc5[1], -1,
                          1)  # The final leg has very low xte tolerance so we actually cross the correct line
            w5.logWaypoint()
        ...
        # Alg sketch: sail a narrow corridor to the middle of the box laterally, 2/3 upwind
            ...
            wToEBrng = (gpsBearing(self.gpsLat1, self.gpsLong1, self.gpsLat4, self.gpsLong4) + gpsBearing(self.gpsLat2,
                                                                                                          self.gpsLong2,
                                                                                                          self.gpsLat3,
                                                                                                          self.gpsLong3)) / 2.0
            sToNBrng = (gpsBearing(self.gpsLat2, self.gpsLong2, self.gpsLat1, self.gpsLong1) + gpsBearing(self.gpsLat3,
                                                                                                          self.gpsLong3,
                                                                                                          self.gpsLat4,
                                                                                                          self.gpsLong4)) / 2.0
            center = gpsVectorOffset(self.gpsLat1, self.gpsLong1,
                                     (wToEBrng - compass_diff(wToEBrng, sToNBrng) / 2) % 360,
                                     math.sqrt(2 * 40 * 40) / 2)  # Follow a diagonal, more or less
            # West location
            ...
            w_center = Waypoint(center[0], center[1], -5.0,
                                5.0)  # We're going to the center, first, so we don't mess up at the beginning
    ...
    def listener(self):
        ...
        normal_checkleg_rate = rospy.Rate(
            0.5)  # Hz  Check every two seconds if far away.  Could clock even slower, but it is the limiting factor on autonomous/manual switching as well
